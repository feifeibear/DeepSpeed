...
class GELU_(nn.Module):
    def forward(self, x):
        return 0.5 * x * (
            1 + torch.tanh(math.sqrt(2 / math.pi) * (x + 0.044715 * torch.pow(x,
                                                                              3))))
...
class Top2Gating(nn.Module):
    ...
    def forward(self, x, importance=None):
        *_, b, group_size, dim = x.shape
        ...
        expert_capacity = min(group_size,
...
class HeirarchicalMoE(nn.Module):
    def __init__(self,
        ...
        self.gate_inner = Top2Gating(dim,
    ...
    def forward(self, inputs, **kwargs):
        ...
        expert_inputs_outer = torch.einsum('bnd,bnec->ebcd',
                                           inputs,
        ...
        expert_inputs = torch.einsum('ebnd,ebnfc->efbcd',
                                     expert_inputs_outer,
        ...
        expert_outputs_outer = torch.einsum('efbcd,ebnfc->ebnd',
                                            expert_outputs,
                                            combine_tensor_inner)
        output = torch.einsum('ebcd,bnec->bnd',
